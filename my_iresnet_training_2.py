...
from torch.autograd import Variable, _tensor_or_tensors_to_tuple
...
def dataLoader(path):
    ...
    print('Train, Val :',len_train,',',len_val)
...
def train_model(model, train_loader,criterion,optimizer,epoch):
    ...
    for idx,(image,label) in enumerate(train_loader):
        ...
        label = nn.functional.one_hot(label,num_classes=1000)
        ...
        if idx%minibatches == 0:
            ...
            print("Train Epoch: {} [{}/{}({:.0f}%)]\tTrain Loss: {:.6f}".format(epoch,idx*len(image),
...
def evaluate_model(model,test_loader,criterion):
    ...
    with torch.no_grad():
        for image,olabel in test_loader:
            image = image.to(device)
            label = nn.functional.one_hot(olabel,num_classes=1000)
            label = label.type(torch.float32).to(device)
            output = model(image)
...
def train():
    path='../Dataset/211022_Data/Images/Training_1_1000/Trainset/sketch/'
    result_path='../Dataset/211022_Data/Images/Training_1_1000/Results/'
    model=load_model()
    ...
    for epoch in tqdm(range(epochs)):
        ...
        print('\n[EPOCCH:{}], \tTest Loss: {:.4f},\tTest Acc: {:.2f} % \n'.format(epoch,test_loss,test_acc))
        train_loss_lst.append(sum(train_loss)/len(train_loss))
        test_loss_lst.append(test_loss.cpu())
